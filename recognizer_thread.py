...
#def search_matches(values,index,batch_size):
#def search_matches(args):
def search_matches(values,new_db):
    #print("args: ",args)
    #values,index,batch_size = args[0], args[1], args[2]
    #print("index of search matches: ",index)  values[index: index + batch_size]
    with new_db.cursor() as cur:
        # Create our IN part of the query
        query = SELECT_MULTIPLE % ', '.join([IN_MATCH] * len(values))
        #print("values[index: index + batch_size]: ",values[index: index + batch_size])
        cur.execute(query, values)   
        cur.flush()      
    return cur.fetchall()#, index
...
def return_matches(hashes, batch_size: int = 1000):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        #args = [[values,index,batch_size] for index in range(0, len(values), batch_size)]
        for index in range(0, len(values), batch_size):
            newdb_cls = get_database(config.get("database_type", "mysql").lower())
            new_db = newdb_cls(**config.get("database", {}))
            thread_results.append(executor.submit(search_matches,values[index: index + batch_size],new_db))
        #thread_results = executor.map(search_matches,args)
        for f in concurrent.futures.as_completed(thread_results):
        #for f in thread_results:
            #some = thread_results[f]
            #print("what is this: ",some)
            matches_count = 0
            try:
                #result,index_t = f
                result = f.result()
            ...
            #print("result of future: ",result)
            #print("index completed: ",index_t)
            ...
            #print(dedup_hashes)
            for res in results:
                #print("SONG ID in results:",res[0])
                if res[0] == 1:
                    print("ID 1",res[0])
            songs_arr = align_matches(results,dedup_hashes,len(hashes))
            print("songs_arr after align_matches: ",songs_arr)
            #print("songs_arr: ",songs_arr[1])
            #if songs_arr[0][HASHES_MATCHED] / 2 > songs_arr[1][HASHES_MATCHED]:
            #    print("apriori time alignement")
            #    break
                #return results, dedup_hashes, songs_arr                
            #else:
            #    songs_arr = []
...
def find_matches(hashes):
    ...
    """
    t = time()
    #matches, dedup_hashes, songs_arr = return_matches(hashes)
    matches, dedup_hashes = return_matches(hashes)
    query_time = time() - t

    return matches, dedup_hashes, query_time#, songs_arr
...
def align_matches(matches, dedup_hashes, queried_hashes,topn: int = TOPN):
    ...
    for song_id, offset, _ in songs_matches[0:topn]:  # consider topn elements in the result
        ...
        song = {
            ...
            #how many sample hashes match song hashes
...
for song_i, song_name in enumerate(songs_to_recognize):
    ...
    #print("hashes: ",hashes)
    #matches, dedup_hashes, query_time, songs_arr = find_matches(hashes)
    matches, dedup_hashes, query_time = find_matches(hashes)
    """
    if len(songs_arr) > 0:
        print("songs_arr has align matches time")
        final_results = songs_arr
        align_time = 0
    else:
    """
    t = time()
